...
#
# Modified from vlash/policies/pi05/modeling_pi05.py (vlash, commit 22cbabf)
# by Dongyoon Kim, 2026. Changes:
#   - Drop "<camera>_mask" columns from config.input_features so normalization
#     leaves the 0/1 flags untouched.
#   - prepare_images: read "<camera>_mask" per sample and use it as the image
#     attention mask instead of always-true.
#   - Load the PaliGemma tokenizer from a local directory
#     (VLASH_PALIGEMMA_PATH) instead of the Hugging Face hub/cache.
...
class PI05Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        # [patch] observation.images.<slot>_mask is a 0/1 camera-valid flag, not a
        # normalization target: prepare_images reads it straight off the batch.
        # Dropping it from input_features keeps normalization away from the flags;
        # the column itself stays in the batch.
    ...
    def prepare_images(self, batch):
        ...
        for key in present_img_keys:
            ...
            bsize = img.shape[0]
            device = img.device
            # [patch] Use the <key>_mask column, when present, as a per-sample
            # valid mask: a dummy camera gets 0, which blocks its attention keys.
            # Without the column every sample is valid, as upstream.
            mkey = f"{key}_mask"
            if mkey in batch:
