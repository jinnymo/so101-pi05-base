...
#
# Modified from lerobot/datasets/video_utils.py (lerobot 0.4.1) by
# Dongyoon Kim, 2026. Changes:
#   - VideoDecoderCache: bound the decoder cache with an LRU of
#     VLASH_DECODER_CACHE_MAX entries (default 64) and close the file handle on
#     eviction. The upstream cache is unbounded and leaks memory across a long
#     run over tens of thousands of videos (upstream issues #2371, #3712).
#   - decode_video_frames_torchcodec: on a decoder error, log the video path and
#     fall back to frame 0 instead of killing the training run.
import glob
import importlib
import logging
import shutil
